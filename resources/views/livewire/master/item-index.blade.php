<div>
    <div class="flex flex-col gap-5 md:flex-row md:items-center md:justify-between mb-6">
        <div class="relative w-full md:w-80">
            ...
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari obat atau BMHP..." class="w-full rounded-lg border border-gray-200 bg-transparent py-2.5 pl-11 pr-4 text-sm outline-none focus:border-brand-500 dark:border-gray-800 dark:bg-transparent">
        </div>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                        <td class="px-4 py-3">
                            <div class="flex flex-col">
                                ...
                                <span class="text-xs text-gray-500">{{ $item->code }} | {{ $item->generic_name ?: '-' }}</span>
                            </div>
                        </td>
                        <td class="px-4 py-3">
                            <span class="text-xs font-medium text-gray-600 dark:text-gray-400">{{ $item->category->name }}</span>
                        </td>
                        <td class="px-4 py-3 text-gray-500">{{ $item->unit->name }}</td>
                        <td class="px-4 py-3">
                            <span class="text-xs text-gray-600 dark:text-gray-400">{{ $item->min_stock }} / {{ $item->max_stock }}</span>
                        </td>
                        <td class="px-4 py-3">
                            @if ($item->is_active)
                        ...
                        <td class="px-4 py-3 text-right">
                            <div class="flex justify-end gap-2">
                                <button wire:click="edit({{ $item->id }})" class="text-gray-400 hover:text-brand-500">
                                ...
                                <button wire:click="$dispatch('confirm-delete', { id: {{ $item->id }}, action: 'delete-item' })" class="text-gray-400 hover:text-red-500">
        ...
        <div class="flex min-h-screen items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-2xl rounded-xl bg-white p-6 shadow-xl dark:bg-gray-900">
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ $isEdit ? 'Ubah Item' : 'Tambah Item Baru' }}</h3>
                ...
                <form wire:submit.prevent="store" class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Kode Item</label>
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="barcode" class="w-full rounded-lg border border-gray-200 bg-transparent px-4 py-2 text-sm outline-none focus:border-brand-500 dark:border-gray-800 dark:bg-transparent">
                        </div>
                    </div>

                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Item</label>
                        <input type="text" wire:model="name" class="w-full rounded-lg border border-gray-200 bg-transparent px-4 py-2 text-sm outline-none focus:border-brand-500 dark:border-gray-800 dark:bg-transparent" placeholder="Contoh: Amoxicillin 500mg">
                        @error('name') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Generik</label>
                        <input type="text" wire:model="generic_name" class="w-full rounded-lg border border-gray-200 bg-transparent px-4 py-2 text-sm outline-none focus:border-brand-500 dark:border-gray-800 dark:bg-transparent">
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Kategori</label>
                            <select wire:model="item_category_id" class="w-full rounded-lg border border-gray-200 bg-transparent px-4 py-2 text-sm outline-none focus:border-brand-500 dark:border-gray-800 dark:bg-transparent">
                                <option value="">Pilih Kategori</option>
                                @foreach ($categories as $cat)
                                    <option value="{{ $cat->id }}">{{ $cat->name }}</option>
                                @endforeach
                            </select>
                            @error('item_category_id') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Satuan Terkecil</label>
                            <select wire:model="item_unit_id" class="w-full rounded-lg border border-gray-200 bg-transparent px-4 py-2 text-sm outline-none focus:border-brand-500 dark:border-gray-800 dark:bg-transparent">
                                <option value="">Pilih Satuan</option>
                                @foreach ($units as $u)
                                    <option value="{{ $u->id }}">{{ $u->name }}</option>
                                @endforeach
                            </select>
                            @error('item_unit_id') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" wire:model="min_stock" class="w-full rounded-lg border border-gray-200 bg-transparent px-4 py-2 text-sm outline-none focus:border-brand-500 dark:border-gray-800 dark:bg-transparent">
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Stok Maksimum</label>
                            <input type="number" wire:model="max_stock" class="w-full rounded-lg border border-gray-200 bg-transparent px-4 py-2 text-sm outline-none focus:border-brand-500 dark:border-gray-800 dark:bg-transparent">
                        </div>
                    </div>

                    <div class="flex flex-wrap gap-4">
                        <div class="flex items-center gap-2">
                            <input type="checkbox" wire:model="is_prescription" id="is_prescription" class="h-4 w-4 rounded border-gray-300 text-brand-600 focus:ring-brand-500">
                            <label for="is_prescription" class="text-sm text-gray-700 dark:text-gray-300">Resep Dokter</label>
                        </div>
                        <div class="flex items-center gap-2">
    ...
    <div x-data="{ open: @entangle('isImportModalOpen') }" x-show="open" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div class="flex min-h-screen items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-lg rounded-xl bg-white p-6 shadow-xl dark:bg-gray-900">
                <div class="mb-4 flex items-center justify-between">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Import Item Obat & BMHP via Excel</h3>
                    <button @click="open = false" class="text-gray-400 hover:text-gray-500">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M18 6L6 18M6 6L18 18" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                    </button>
                </div>
                ...
                <div class="mb-6">
                    <div class="flex items-center justify-between mb-2">
                        <p class="text-sm text-gray-500 italic text-[10px]">Format: <b>kode, nama, nama_generik, kode_kategori, kode_satuan, ...</b></p>
                        <button wire:click="downloadTemplate" class="text-xs font-medium text-brand-500 hover:text-brand-600 flex items-center gap-1 shrink-0">
                    ...
                    <div class="flex items-center justify-center w-full">
                        <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                    ...
                    @error('importFile') <span class="mt-1 text-xs text-red-500">{{ $message }}</span> @enderror
