<div class="max-w-5xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-black text-gray-900 uppercase tracking-tight">Proses Pengiriman Stok</h2>
                <span class="text-[10px] font-black uppercase tracking-widest text-gray-400 mt-1 block">Fulfillment & Batch Allocation (FEFO)</span>
            </div>
        </div>

        <div class="text-right">
            <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-1">No. Transaksi</span>
            <span class="text-sm font-black text-indigo-600 italic bg-indigo-50 px-3 py-1 rounded-lg border border-indigo-100">{{ $distribution->distribution_number }}</span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm col-span-2">
            <div class="flex items-center justify-between mb-6">
                <div class="flex-1">
                    <span class="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] block mb-2">Gudang Pengirim (Asal)</span>
                ...
                <div class="flex-1 text-right">
                    <span class="text-[9px] font-black text-gray-400 uppercase tracking-[0.2em] block mb-2">Unit Penerima (Tujuan)</span>
            ...
            <div class="p-4 bg-gray-50 rounded-xl border border-gray-100">
                <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest block mb-2 italic">Catatan Permintaan:</span>
                <p class="text-sm font-medium text-gray-600">{{ $distribution->notes ?? 'Tidak ada catatan.' }}</p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
            <h3 class="text-[11px] font-black uppercase tracking-widest text-gray-600">Daftar Barang & Alokasi Batch</h3>
            <span class="px-3 py-1 bg-gray-100 text-[9px] font-black text-gray-400 rounded-lg uppercase italic">{{ count($items) }} Items to Ship</span>
        </div>
            ...
            <table class="w-full text-left text-[11px]">
                <thead>
                    <tr class="bg-gray-50/50 border-b border-gray-100">
                        ...
                        <th class="px-6 py-4 font-black text-gray-400 uppercase text-center w-24">Permintaan</th>
                    @forelse($items as $index => $item)
                        <tr class="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="font-black text-gray-600 text-lg">{{ number_format($item['qty_requested']) }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <select wire:model.live="items.{{ $index }}.item_batch_id" class="w-full bg-gray-50 border-gray-100 rounded-lg text-xs px-3 py-2.5 font-bold text-gray-700 focus:ring-brand-500 focus:border-brand-500 transition-all">
                                ...
                                @error('items.'.$index.'.item_batch_id') <span class="text-[8px] text-red-500 font-bold uppercase block mt-1">Wajib pilih batch</span> @enderror
                            </td>
                             <td class="px-6 py-4 text-center">
                                <span class="font-black text-emerald-600 text-lg italic">{{ number_format($item['destination_stock']) }}</span>
                                <span class="text-[8px] font-black text-gray-400 uppercase block tracking-tighter">Stok Tujuan</span>
                            </td>
                            <td class="px-6 py-4">
                                <input type="number" wire:model="items.{{ $index }}.qty_sent" step="1" class="w-full bg-indigo-50 border-indigo-100 rounded-lg text-sm px-3 py-2 text-center font-black text-indigo-600 focus:ring-indigo-500 focus:border-indigo-500 transition-all shadow-inner">
                                @error('items.'.$index.'.qty_sent') <span class="text-[8px] text-red-500 font-bold uppercase block mt-1">Invalid</span> @enderror
                            </td>
                        </tr>
                        <tr class="bg-gray-50/20">
                            <td colspan="5" class="px-6 py-2 border-b border-gray-100">
                                <div class="flex items-center gap-2">
                                    <span class="text-[8px] font-black text-gray-400 uppercase italic">Catatan Item:</span>
                                    <input type="text" wire:model="items.{{ $index }}.notes" placeholder="Misal: Stok sisa sedikit, baru dikirim sebagian..." class="flex-1 bg-transparent border-none text-[10px] font-medium text-gray-500 placeholder:text-gray-300 focus:ring-0 py-0">
                                </div>
                            </td>
                        </tr>
                    @empty
