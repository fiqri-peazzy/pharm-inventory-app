<div>
    <div class="flex flex-col gap-5 md:flex-row md:items-center md:justify-between mb-6">
        <div class="flex flex-1 items-center gap-4">
            <div class="relative w-full md:w-80">
                ...
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari No. PR..."
                    class="w-full rounded-lg border border-gray-200 bg-white py-2.5 pl-11 pr-4 text-sm outline-none focus:border-brand-500 dark:border-gray-800 dark:bg-gray-900 transition-all">
            </div>

            <select wire:model.live="status"
                class="rounded-lg border border-gray-200 bg-white py-2.5 px-4 text-sm outline-none focus:border-brand-500 dark:border-gray-800 dark:bg-gray-900 transition-all">
        ...
        <a href="{{ route('procurement.requests.create') }}"
            class="flex items-center gap-2 rounded-lg bg-brand-500 px-5 py-2.5 text-sm font-bold text-white hover:bg-brand-600 shadow-lg shadow-brand-500/20 transition-all uppercase tracking-wider">
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div
        class="overflow-hidden bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm transition-all">
                    ...
                    <tr class="hover:bg-gray-50/50 dark:hover:bg-white/[0.01] transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex flex-col">
                                <span
                                    class="font-bold text-gray-900 dark:text-white group-hover:text-brand-500 transition-colors uppercase tracking-tight">{{ $request->request_number }}</span>
                                <span
                                    class="text-[11px] text-gray-500 italic">{{ $request->request_date->format('d M Y') }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2">
                                ...
                                <span
                                    class="text-xs font-medium text-gray-700 dark:text-gray-300">{{ $request->warehouse->name }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span class="text-xs font-bold text-gray-600 dark:text-gray-400 capitalize">
                                {{ \Carbon\Carbon::createFromDate($request->period_year, (int) $request->period_month, 1)->translatedFormat('F') }}
                                {{ $request->period_year }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            ...
                            @endphp
                            <span
                                class="inline-flex items-center rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-widest {{ $color }} ring-1 ring-inset shadow-sm">
                                {{ $request->status }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2" wire:key="actions-{{ $request->id }}">
                                @if (in_array($request->status, ['draft', 'rejected']))
                                    <a href="{{ route('procurement.requests.edit', $request->id) }}"
