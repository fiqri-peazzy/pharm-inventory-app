    ...
    <div class="flex flex-col gap-5 md:flex-row md:items-center md:justify-between mb-6">
        <div class="flex flex-1 flex-wrap items-center gap-3">
            <div class="relative w-full md:w-64">
                ...
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari No. PO..."
        ...
        <a href="{{ route('procurement.orders.create') }}"
            class="flex items-center gap-2 rounded-xl bg-brand-500 px-6 py-2.5 text-xs font-black text-white hover:bg-brand-600 shadow-lg shadow-brand-500/20 transition-all uppercase tracking-widest">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                stroke-width="2.5">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors group">
                        <td class="px-6 py-4">
                            <div class="flex flex-col">
                                <span
                                    class="font-bold text-gray-900 dark:text-white group-hover:text-brand-500 uppercase tracking-tight">{{ $order->po_number }}</span>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-col">
                                <span
                                    class="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-tight">{{ $order->supplier->name ?? 'Internal Store' }}</span>
                                <span
                                    class="text-[10px] font-bold text-indigo-500 uppercase tracking-widest">{{ $order->warehouse->name }}</span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-center">
                            <span
                                class="text-[11px] font-bold text-gray-600 dark:text-gray-400">{{ \Carbon\Carbon::parse($order->po_date)->format('d/m/Y') }}</span>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <span
                                class="text-[13px] font-black text-gray-900 dark:text-white leading-none">Rp{{ number_format($order->grand_total) }}</span>
                        </td>
                        <td class="px-6 py-4 text-center">
                            @php
                                $statusColors = [
                            ...
                            @endphp
                            <span
                                class="inline-flex items-center rounded-full px-2.5 py-1 text-[9px] font-black uppercase tracking-widest {{ $color }} ring-1 ring-inset shadow-sm">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex justify-end gap-2" wire:key="po-actions-{{ $order->id }}">
                                @if (in_array($order->status, ['draft', 'sent']))
                                    <a href="{{ route('procurement.orders.edit', $order->id) }}"
                                        class="p-2 text-gray-300 hover:text-brand-500 hover:bg-brand-50 rounded-xl transition-all">
                                ...
                                @endif
                                <a href="{{ route('procurement.orders.print', $order->id) }}" target="_blank"
                                    class="p-2 text-gray-300 hover:text-blue-500 hover:bg-blue-50 rounded-xl transition-all">
                                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                                        stroke="currentColor" stroke-width="2">
                                ...
                                @if (auth()->user()->hasPermissionTo('purchase-orders.approve') && $order->status === 'submitted')
                                    ...
                                    <button wire:click="$dispatch('confirm-reject-po', { id: {{ $order->id }} })"
                                ...
                                @if (auth()->user()->hasPermissionTo('purchase-orders.direktur-approve') && $order->status === 'pending_director')
                                    <button wire:click="approve({{ $order->id }})"
                                        class="p-2 text-purple-400 hover:text-purple-600 hover:bg-purple-50 rounded-xl transition-all"
                                        title="Direktur Approve">
                                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none"
                                            stroke="currentColor" stroke-width="2.5">
                                            <path
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-20 text-center opacity-40 italic text-gray-400 text-sm">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <span class="text-xs uppercase font-black tracking-widest">Belum ada data pesanan
                                    (PO)</span>
                            </div>
                        </td>
                    </tr>
