...
class DisposalForm extends Component
{
    ...
    public function post()
    {
        ...
        try {
            ...
            // Accounting Integration (Auto-Posting Disposal)
            try {
                ...
                foreach ($disposal->details as $detail) {
                    // Only post journal if this disposal actually deducted stock (not from source)
                    if (empty($detail->source_type)) {
                        $inventoryAccount = $accountingService->getInventoryAccountByCategory($detail->item->category?->type);
                        $disposalLossAcc = $accountingService->getDisposalLossAccount();
                        ...
                        // Debit Loss
                        if (!isset($summaryByAccount['loss_' . $disposalLossAcc->id])) {
                            $summaryByAccount['loss_' . $disposalLossAcc->id] = ['account' => $disposalLossAcc, 'debit' => 0, 'credit' => 0];
                        }
                        $summaryByAccount['loss_' . $disposalLossAcc->id]['debit'] += $amount;
                    }
                }
                ...
                if (count($entries) > 0) {
                    $accountingService->createJournalEntry([
                        'journal_number' => $disposal->disposal_number,
                        'journal_date' => now(),
                        'type' => 'adjusting',
                        'transaction_type' => 'disposal',
                        'transaction_id' => $disposal->id,
                        'description' => 'Auto-journal for disposal ' . $disposal->disposal_number,
                        'status' => 'posted',
                        'entries' => $entries
                    ]);
                }
                ...
                \Log::error('Accounting auto-posting failed for disposal ' . $disposal->disposal_number . ': ' . $e->getMessage());
            }
